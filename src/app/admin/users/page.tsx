			...
			{users
				.filter(
					(user) =>
						...
						(user.user_metadata.name?.toLowerCase() ?? "").includes(
							searchTerm.toLowerCase()
						)
				)
				.map((user, index) => {
					return (
						<a key={user.id} href={"users/" + user.id}>
							<div
								key={index}
								className="my-1 rounded-3xl p-4 hover:bg-[#4D956D] hover:text-[#FCFBE4] w-full transition-all border-4 m-4 duration-1000"
							>
